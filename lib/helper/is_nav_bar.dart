...
import 'package:flutter_fluffychat_ios/theme/is_theme.dart';
import 'package:flutter_fluffychat_ios/theme/theme_switcher.dart';
...
const double _kNavBarShowLargeTitleThreshold = 25.0;
...
Widget _wrapWithBackground(
    {Border border, Color backgroundColor, Brightness brightness, Widget child, bool updateSystemUiOverlay = true, bool isBlurred = true}) {
  Widget result = child;
  ...
  return ClipRect(
    child: BackdropFilter(
      filter: (isBlurred) ? ImageFilter.blur(sigmaX: 10.0, sigmaY: 10.0) : ImageFilter.blur(),
...
/// to also has a [ISNavigationBar] or a [ISSliverNavigationBar]
...
///
/// By default, only one [ISNavigationBar] or [ISSliverNavigationBar]
...
///  * [ISSliverNavigationBar] for a navigation bar to be placed in a
///    scrolling list and that supports iOS-11-style large titles.
class ISNavigationBar extends StatefulWidget implements ObstructingPreferredSizeWidget {
  /// Creates a navigation bar in the iOS style.
  const ISNavigationBar({
    Key key,
    this.leading,
  ...
  ///
  /// Defaults to a common tag between all [ISNavigationBar] and
  /// [ISSliverNavigationBar] instances of the same [Navigator]. With the
  ...
  @override
  _ISNavigationBarState createState() {
    return _ISNavigationBarState();
  }
}
...
class _ISNavigationBarState extends State<ISNavigationBar> {
...
///
/// The [ISSliverNavigationBar] must be placed in a sliver group such
...
///
/// Like [ISNavigationBar], it also supports a [leading] and [trailing]
...
/// See also:
///
///  * [ISNavigationBar], an iOS navigation bar for use on non-scrolling
///    pages.
class ISSliverNavigationBar extends StatefulWidget {
  /// Creates a navigation bar for scrolling lists.
  ///
  /// The [largeTitle] argument is required and must not be null.
  const ISSliverNavigationBar({
    Key key,
    this.largeTitle,
    this.strokeTitle,
    this.leading,
  ...
  @override
  _ISSliverNavigationBarState createState() => _ISSliverNavigationBarState();
}

// A state class exists for the nav bar so that the keys of its sub-components
// don't change when rebuilding the nav bar, causing the sub-components to
// lose their own states.
class _ISSliverNavigationBarState extends State<ISSliverNavigationBar> {
  _NavigationBarStaticComponentsKeys keys;
  ...
  Widget build(BuildContext context) {
    ...
    final _NavigationBarStaticComponents components = _NavigationBarStaticComponents(
      ...
      userStrokeTitle: widget.strokeTitle,
      padding: widget.padding,
      large: true,
    );
...
class _LargeTitleNavigationBarSliverDelegate extends SliverPersistentHeaderDelegate with DiagnosticableTreeMixin {
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    ...
    final _PersistentNavigationBar persistentNavigationBar = _PersistentNavigationBar(
      ...
      actionColor: actionsForegroundColor,
    );
...
class _PersistentNavigationBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (leading == null && backChevron != null && backLabel != null) {
      leading = CupertinoNavigationBarBackButton._assemble(backChevron, backLabel, actionColor);
    }
...
class _NavigationBarStaticComponentsKeys {
  _NavigationBarStaticComponentsKeys()
      : navBarBoxKey = GlobalKey(debugLabel: 'Navigation bar render box'),
        ...
        strokeTitleKey = GlobalKey(debugLabel: 'Stroke title');
  ...
  final GlobalKey strokeTitleKey;
}
...
class _NavigationBarStaticComponents {
  _NavigationBarStaticComponents({
    ...
    @required Widget userStrokeTitle,
    ...
    padding: padding,
        ...
        strokeTitle = createStrokeTitle(
          strokeTitleKey: keys.strokeTitleKey,
          userStrokeTitle: userStrokeTitle,
        );
  ...
  /// This widget is not decorated with a font since the font style could
  /// animate during transitions.
  final KeyedSubtree strokeTitle;

  static KeyedSubtree createStrokeTitle({
    @required GlobalKey strokeTitleKey,
    @required Widget userStrokeTitle,
  }) {
    final Widget strokeTitleContent = userStrokeTitle;

    assert(
    strokeTitleContent != null,
    'strokeTitle was not provided and there was no title from the route.',
    );

    return KeyedSubtree(
      key: strokeTitleKey,
      child: strokeTitleContent,
    );
  }
}

/// A nav bar back button typically used in [ISNavigationBar].
///
/// This is automatically inserted into [ISNavigationBar] and
/// [ISSliverNavigationBar]'s `leading` slot when
/// `automaticallyImplyLeading` is true.
///
...
class CupertinoNavigationBarBackButton extends StatelessWidget {
  ...
  const CupertinoNavigationBarBackButton._assemble(this._backChevron, this._backLabel, this.color)
